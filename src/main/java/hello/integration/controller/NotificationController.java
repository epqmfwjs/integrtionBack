...
public class NotificationController {
    ...
    public ResponseEntity<String> registerToken(@RequestBody TokenRegistrationRequest request) {
        ...
        log.info("디버그: 현재 등록된 토큰 수: {}", notificationService.getCurrentTokenInfo());
        return ResponseEntity.ok("Token registered successfully");
    }

    // 디버깅용 엔드포인트
    @GetMapping("/tokens")
    public ResponseEntity<String> getRegisteredTokens() {
        return ResponseEntity.ok(notificationService.getCurrentTokenInfo());
    }
